...
const DetailPage = () => {
  ...
  useEffect(() => {
    if (!isDateXAfterDateY(dateRange.startDate, dateRange.endDate)) {
      setDataPrice(() => {
        ...
        return {
          ...
          total: Number.parseFloat(
            (numberOfDay * (homeStay?.price || 0)).toFixed(2)
          ),
        };
      });
    }
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <Stack.Screen
        options={{
          header: () => (
            <SafeAreaView style={{ ...styles.card, padding: 20 }}>
...
function generateMarkedDates(startDateString: string, endDateString: string) {
