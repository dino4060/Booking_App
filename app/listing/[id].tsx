...
const DetailsPage = () => {
  ...
  const handleLoveButtonClick = async () => {
    ...
    if (type == "no") {
      ...
      // call api remove
      await WishlistHandle.removeFromWishList(room_id as string);
      setType("no");
    }
  };

  const handleWishlist = async () => {
    const res: Wishlist[] = await WishlistHandle.getWishList();
    const idList = res.map((x) => x.room._id);
    const checkExit = idList.includes(room_id as string) ? "yes" : "no";
    ...
    navigation.setOptions({
      ...
      headerRight: () => (
        <View style={styles.bar}>
          <TouchableOpacity style={styles.roundButton} onPress={shareRoom}>
      ...
      >
        ...
        <View style={styles.infoContainer}>
          ...
          <Text style={styles.location}>
            {homeStay?.room_type} in {homeStay?.smart_location}
          </Text>
          <Text style={styles.rooms}>
            {homeStay?.bedRooms} bedrooms · {homeStay?.beds} bed ·{" "}
          ...
          <View style={styles.hostView}>
            <TouchableOpacity
              onPress={() => router.push(`/host/${homeStay?.host?._id}`)}
            >
              <Image
                source={{
            ...
            <View>
              ...
              <Text
                onPress={() => console.log(homeStay?.host?.created_at)}
                style={{
                  fontSize: 14,
                  fontFamily: "mon-sb",
                }}
          ...
          <View style={styles.detail}>
            ...
            <View>
              ...
              <Text style={styles.description}>{homeStay?.summary}</Text>
            </View>
          </View>
          ...
          <View style={{ ...styles.detail, marginTop: 12 }}>
            ...
            <View>
              ...
              <Text style={styles.description}>{homeStay?.transit}</Text>
            </View>
          </View>
      ...
      >
        ...
        >
          <TouchableOpacity style={styles.footerText}>
            <Text style={styles.footerPrice}>€{homeStay?.price}</Text>
            <Text>night</Text>
          </TouchableOpacity>

          <TouchableOpacity
            onPress={() => router.push(`/reservation/${homeStay?._id}`)}
